...
SELECT
    ...
    (COALESCE(processing_fee, 0) * total_amount_items)/6
...
/* REFUND REPORT - raw */
DROP TABLE IF EXISTS reporting_sch.refund_report_raw;

CREATE TABLE reporting_sch.refund_report_raw AS
SELECT
    p.order_id,
    p.order_item_id,
    p.order_number,
