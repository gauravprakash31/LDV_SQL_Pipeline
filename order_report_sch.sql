...
SELECT
    ...
    COALESCE(ocp.order_level_refund, 0)                 AS order_level_refund,
    ...
    (COALESCE(ocp.processing_fee, 0) * COALESCE(total_amount_items, 0) / NULLIF(ocp.original_amount, 0)) AS create_line_item_processing_fee,
    (COALESCE(ocp.refunded_processing_fee, 0) * COALESCE(total_amount_items, 0) / NULLIF(ocp.total, 0))  AS refund_line_item_processing_fee,
    ...
    COALESCE(ocp.refunded_processing_fee, 0)            AS refunded_processing_fee,
    COALESCE(ocp.total_amount_items, 0)                 AS total_amount_items,
